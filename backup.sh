...
# Getting public key from the age key file
PUBLIC_KEY="$(cat "${AGE_KEY_FILE}" | awk 'match($0, /^# *public key: */) {gsub(/^# *public key: */, ""); print}')"
...
function backup_rsakey() {
    ...
    if [[ -d "${DATA_RSAKEY_DIRNAME}" ]]; then
        local FIND_RSAKEY
        FIND_RSAKEY=$(find "${DATA_RSAKEY_DIRNAME}" -name "${DATA_RSAKEY_BASENAME}*" -print0 | xargs -0 -I {} basename {})
...
function backup_package() {
    ...
    echo "package backup file and encryption with age"


    tar -czf - "${BACKUP_DIR}"/*  | age -e -r "$PUBLIC_KEY" > "$BACKUP_FILE_GZIP"
